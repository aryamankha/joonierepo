...
But measurement itself carries assumptions. When we measure productivity, we're implicitly valuing output over process. When we measure engagement, we're prioritizing attention over meaning. The metrics we choose shape the systems we build, and AI will only accelerate this feedback loop. Perhaps the most important question isn't what AI can optimize, but what we should resist optimizing in the first place.

There's also a temporal dimension to consider. The benefits of AI integration compound differently across time horizons. In the short term, we gain efficiency. In the medium term, we reshape workflows and job functions. But in the long term, we may fundamentally alter what it means to develop expertise, to struggle through problems, to earn understanding through effort. If AI shortcuts the journey, do we lose something essential about the destination?

I keep returning to the idea that the most profound changes won't be the ones we anticipate. The internet didn't just make communication faster; it rewired social structures, attention spans, and even our sense of self. AI will likely do something similar, not by doing what we expect it to do better, but by enabling entirely new behaviors and relationships we haven't yet imagined. The macro effects, by definition, will surprise us.

And maybe that's the point. Maybe the value of thinking about macro effects isn't to predict them accurately, but to cultivate a certain humility and openness. To build systems and institutions that can adapt rather than ones optimized for a future we think we understand. The best we can do might not be to plan for specific outcomes, but to remain curious, flexible, and willing to be wrong about everything we thought we knew.
